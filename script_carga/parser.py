...
def parse(filename):
	try:
		...
		if str(sh.cell_value(rowx=0, colx=0)) == 'A':
			...
			values.append(sh.cell_value(rowx=4, colx=5))#20 ALIMENTACION ESPECIAL
			values.append(sh.cell_value(rowx=5, colx=5))#21 Comentarios
			values.append(sh.cell_value(rowx=6, colx=5))#22 Recomendacion
